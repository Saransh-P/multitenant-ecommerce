  ...
  return (
    <Button 
        ...
        className={cn ( "bg-transparent hover:bg-transparent rounded-full hover:border-primary border-transparent px-3.5 text-lg",
            isActive && "text-white bg-black hover:bg-black hover:text-white"
        )}>
            <Link href={href}>
                {children}
            </Link>
  ...
  return (
    <nav className="h-20 flex border-b justify-between font-medium bg-white">
        ...
        <NavbarSidebar 
            ...
            items={navbarItemsName} 
        />

        <div className="items-center gap-4 hidden lg:flex">
            {navbarItemsName.map((item) => (
                <NavbarItem
                    key={item.href}
                    href={item.href}
        ...
        <div className="hidden lg:flex">
            ...
            <Button
                asChild
                className="border-l border-t-0 border-b-0 border-r-0 rounded-none px-12 h-full bg-black text-white hover:bg-pink-400 hover:text-black transition-colors text-lg"
            >
